...
export function initializeGroups(students: Student[], numberOfGroups: number): Student[][] {
    shuffle(students);
    const nodeGroups = dfsGrouping(students, numberOfGroups);
    const studentGroups = nodeGroups.map(group => group.map(node => node.student));
...
function dfsGrouping(students: Student[], numberOfGroups: number): Node[][] {
    let nodes: Record<number, Node> = students.reduce((acc: Record<number, Node>, student) => {
    ...
    const ungroupedNodes = Object.values(nodes).filter(node => node.color !== 'Red');
...
function getGroup(node: Node, targetSize: number, limit: number, depth: number = 0, partial: Node[] = []): Node[] {
    ...
    for (const neighbor of node.neighbors) {
        if (neighbor.color !== 'Red' && !currentGroup.includes(neighbor)) {
            const result = getGroup(neighbor, targetSize, limit, depth + 1, currentGroup);
...
function isValidGroup(group: Node[]): boolean {
    ...
    return group.every(node => node.neighbors.some(neighbor => groupSet.has(neighbor)));
}
